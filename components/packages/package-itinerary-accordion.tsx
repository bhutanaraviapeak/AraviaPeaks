...
import { Utensils, BedDouble, MapPin, Plus, ShieldCheck } from "lucide-react"
...
// Verified-real photos only (several root-level /public images are AI-generated —
// see site-audit notes). Ordered keyword → image rules; first match wins.
const IMAGE_RULES: { re: RegExp; img: string | string[] }[] = [
  ...
  { re: /dochula|dochu la/, img: "/bhutan-dochula-pass-108-chortens-himalayan-mountain.jpg" },
  { re: /buddha|dordenma/, img: "/bhutan-buddha-dordenma-giant-golden-statue-thimphu.jpg" },
  { re: /crane|bird|wildlife|phobjikha/, img: "/phobjikha-valley-glacial-bhutan-black-necked-crane.jpg" },
  ...
  { re: /monk|study|lhakhang|temple|shrine|chapel|nunnery/, img: `${I}monks.jpg` },
  ...
  { re: /bumthang|jakar/, img: `${P}jambay-lhakhang.jpg` },
...
// Days that declare an explicit image in the data always keep it.
function assignImages(items: TimelineItem[]): string[] {
  const out: string[] = []
  for (let i = 0; i < items.length; i++) {
    if (items[i].image) {
      out.push(items[i].image as string)
      continue
    }
    const text = `${items[i].title || ""} ${items[i].description || ""}`.toLowerCase()
    let img = resolveImage(text, i)
    if (i > 0 && img === out[i - 1]) {
...
// A short journey-stage label per day, derived from the day's own content so it
// reads like a story beat rather than a generic "Exploration" tag.
function phaseLabel(item: TimelineItem, total: number): string {
  const text = `${item.title} ${item.description}`.toLowerCase()
  if (item.dayNumber <= 1 || /arriv|welcome/.test(text)) return "Arrival"
  if (item.dayNumber >= total || /depart|farewell/.test(text)) return "Farewell"
  if (/festival|tshechu|drubchen|cham|mewang|fire blessing/.test(text)) return "Festival day"
  if (/rest day|acclimatis|acclimatiz/.test(text)) return "Rest & acclimatise"
  if (/trek|pass|camp|trail|la \(|ridgeline/.test(text)) return "On the trail"
  if (/spa|wellness|yoga|hot.?stone|massage/.test(text)) return "Wellness"
  if (/meditat|prayer|blessing|pilgrim|monaster|nunnery|lhakhang/.test(text)) return "Sacred"
  if (/photograph|shoot|frame/.test(text)) return "Behind the lens"
  if (/bird|crane|wildlife/.test(text)) return "In the wild"
  return "Discover"
}
...
export function PackageItineraryAccordion({
  ...
  const normalizedItems = useMemo<TimelineItem[]>(() => {
    ...
    // Fully bespoke journeys (no fixed duration) get no generated filler days —
    // the empty state below explains that the plan is designed after inquiry.
  ...
  }, [safeItinerary, fallbackDays, safeHighlights, packageTitle, durationDays])
  ...
  if (!items.length) {
    return (
      <div className="rounded-2xl border border-dashed border-accent/40 bg-accent/5 p-6 text-center">
        <p className="font-serif text-lg font-semibold text-foreground">Your itinerary, written for you</p>
        <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
          A bespoke journey has no fixed days. Tell us your dates, pace, and interests — we&rsquo;ll design a
          day-by-day plan around you and refine it together until it&rsquo;s exactly right.
        </p>
      </div>
    )
  }
                ...
                <div className="overflow-hidden rounded-2xl border border-border/70 bg-card transition-all duration-300 group-hover/day:border-accent/40 group-hover/day:shadow-[0_20px_44px_-26px_rgba(18,37,54,0.4)] group-focus-within/day:border-accent/40">
                  ...
                  <div className="flex items-center gap-4 px-5 py-4">
                    <div className="min-w-0 flex-1">
                      ...
                      {/* One-line teaser so the collapsed list is scannable */}
                      ...
                      <div className="px-5 pb-6">
                        ...
                        <div className={cn("grid gap-5 md:grid-cols-2 md:items-stretch", imageRight && "md:[&>*:first-child]:order-2")}>
                          <div className="relative min-h-48 overflow-hidden rounded-xl md:min-h-full">
                            ...
                            {item.location && (
                              <span className="absolute bottom-3 left-3 inline-flex max-w-[calc(100%-1.5rem)] items-center gap-1.5 rounded-full bg-black/45 px-2.5 py-1 text-[11px] font-medium text-white backdrop-blur-sm">
